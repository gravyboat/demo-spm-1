...
Vagrant.configure('2') do |config|
  ...
  config.vm.synced_folder('salt/roots/salt', '/srv/salt/demo-spm-1')
  config.vm.synced_folder('salt/pillar_data', '/srv/pillar')

  config.vm.provision "shell", inline: "sudo yum update -y nss\*"

  config.vm.define "demo-spm-1_0" do |node|
    node.vm.network 'private_network', ip: '192.168.44.3'
    node.vm.synced_folder './', '/home/vagrant/demo-spm-1'

    node.vm.provision(:salt) do |s|
      s.install_type = "git"
      s.install_args = "v2015.8.0rc1"
      s.verbose       = true
      s.minion_config = 'salt/minion/demo_spm_1'
